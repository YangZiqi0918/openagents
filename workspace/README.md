...
The development Compose stack is a local single-user deployment: it opens a
local workspace without an OpenAgents account, uses only the local API, and
disables Firebase, analytics, Yumi, and the hosted campaign. All published ports
are bound to loopback. Do not expose this mode to an untrusted network: local
access grants owner-level workspace credentials without authentication.
...
the local backend. The backend must run with `LOCAL_MODE=true` too. Changes to
`NEXT_PUBLIC_*` require restarting the dev server or rebuilding the frontend.
Hosted deployments should leave `LOCAL_MODE` disabled and explicitly set
`NEXT_PUBLIC_LOCAL_MODE=false` when building their frontend.
