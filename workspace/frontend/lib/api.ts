...
export class WorkspaceApi {
  ...
  async listConversations(agentFilter?: string): Promise<DMConversation[]> {
    const all: DMConversation[] = [];
    const limit = 100;
    const seen = new Set<string>();
    let offset = 0;
    while (true) {
      const params = new URLSearchParams({ network: this.workspaceId, limit: String(limit), offset: String(offset) });
      ...
      for (const conversation of result.conversations) {
        const key = [...conversation.agents].sort().join(',');
      ...
      if (result.conversations.length < limit || added === 0) return all;
    }
  }
