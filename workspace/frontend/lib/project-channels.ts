...
export function projectShareUrl(
  ...
  const url = new URL(`/${encodeURIComponent(workspaceId)}`, origin);
  url.searchParams.set('projectId', link.projectId);
  url.searchParams.set('projectName', link.projectName);
  if (link.sessionId && belongsToProject(link.sessionId, link.projectId)) {
    url.searchParams.set('projectSessionId', link.sessionId);
  }
  return url.toString();
}
