...
interface WorkspaceContextValue {
  ...
  createAndSendSession: (input: { content: string; participants: string[]; files: File[] }) => Promise<WorkspaceSession>;
...
export function WorkspaceProvider({
  ...
  const createAndSendSession = useCallback(async ({ content, participants, files }: {
    content: string; participants: string[]; files: File[];
  }) => {
    const title = content.trim().split(/\r?\n/).find(Boolean)?.slice(0, 60) || files[0]?.name || 'New Thread';
    pendingFirstSendsRef.current += 1;
    try {
      const session = await workspaceApi.createChannel({ title, participants });
      try {
        const uploaded = await Promise.all(files.map((file) => workspaceApi.uploadFile(file, session.sessionId)));
        ...
        await workspaceApi.updateChannel(session.sessionId, { status: 'deleted' }).catch(() => {});
        throw error;
      }
      setSessions((current) => current.some((item) => item.sessionId === session.sessionId)
        ...
        : [session, ...current]);
      setCurrentSessionId(session.sessionId);
      capture('thread_created', { participant_count: participants.length, has_resume: false });
      capture('message_sent', { has_attachments: files.length > 0, attachment_count: files.length });
      return session;
    } finally {
      pendingFirstSendsRef.current -= 1;
    }
  }, [currentUser.id, currentUser.name, setCurrentSessionId]);
