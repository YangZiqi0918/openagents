...
import {
  BookOpen, CalendarClock, ChevronDown, ChevronLeft, ChevronRight, FileText, FolderKanban, Globe,
  Inbox, KanbanSquare, Monitor, PlusSquare, Sparkles, SquarePen, Users, Waypoints,
} from 'lucide-react';
import {
  Sidebar,
  SidebarContent,
...
const HIDDEN_NAV_MODES = new Set<ViewMode>(['files', 'browser', 'tasks', 'workflows']);
const SHOW_ONLINE_PEOPLE = false;
...
export function NavRail() {
  const {
    viewMode, openView, openNewThread, draftThreadOpen, setSelectedAgentName, setDraftThreadOpen, isRailExpanded, railDragWidth,
  } = useLayout();
  const {
    workspace, agents, unreadNotificationCount,
  ...
  return (
    ...
    >
      ...
      <SidebarContent>
        <SidebarGroup className="px-1.5">
          ...
          <SidebarGroupContent>
            <SidebarMenu className="gap-0.5">
              {items.filter((item) => !item.mode || !HIDDEN_NAV_MODES.has(item.mode)).map((item) => {
                ...
                return <SidebarMenuItem key={item.mode || item.href}>
                  <SidebarMenuButton
                    asChild={!!item.href}
                    className={cn('relative', !showLabels && 'justify-center!')}
                    aria-label={item.label}
                    tooltip={{ children: item.label, hidden: showLabels }}
                    isActive={mode === 'threads' ? viewMode === 'threads' && draftThreadOpen : viewMode === mode}
                    onClick={mode === 'threads' ? openNewThread : mode ? () => openView(mode) : undefined}
                  >
        ...
        {/* Agents remain available on demand, but start collapsed. */}
        {recentAgents.length > 0 && (
          <>
            ...
            <SidebarGroup className="px-1.5">
              ...
              <SidebarGroupContent
                id="rail-agent-list"
                className={cn(!agentsOpen && 'hidden')}
              >
                <SidebarMenu className="gap-0.5">
        ...
        <AllConversationList showLabels={showLabels} />
        ...
        {SHOW_ONLINE_PEOPLE && onlineUsers.length > 0 && (
          <>
            <div className="px-3">
              <Separator />
            </div>
