...
import { Archive, ArchiveRestore, MessageSquare, MoreHorizontal, PanelLeft, Pencil, Star, Trash2 } from "lucide-react"
...
function ThreadActions() {
  const { currentSessionId, sessions, renameSession, updateSession, setCurrentSessionId } = useWorkspace()
  const { openNewThread } = useLayout()
  const prompt = usePrompt()
  const confirm = useConfirm()
  const t = useT()
  const session = sessions.find((item) => item.sessionId === currentSessionId)
  if (!session || currentSessionId?.startsWith('dm:')) return null
  ...
  const rename = async () => {
    ...
    if (title && title !== session.title) await renameSession(session.sessionId, title)
  }

  const remove = async () => {
    const ok = await confirm({
      title: t('threads.deleteTitle'),
      description: t('threads.deleteDescription', { title: session.title || t('threads.untitled') }),
      confirmText: t('common.delete'),
      destructive: true,
    })
    if (!ok) return
    await updateSession(session.sessionId, { status: 'deleted' })
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" mode="icon" size="sm" aria-label={t('threads.threadActions')} title={t('threads.threadActions')} className="text-muted-foreground">
          <MoreHorizontal className="size-4" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-44">
        <DropdownMenuItem onClick={() => void rename()}><Pencil className="size-4" />{t('common.rename')}</DropdownMenuItem>
        <DropdownMenuItem onClick={() => void updateSession(session.sessionId, { starred: !session.starred })}><Star className="size-4" />{session.starred ? t('threads.unstar') : t('threads.star')}</DropdownMenuItem>
        <DropdownMenuItem onClick={() => void updateSession(session.sessionId, { status: session.status === 'archived' ? 'active' : 'archived' })}>
          {session.status === 'archived' ? <ArchiveRestore className="size-4" /> : <Archive className="size-4" />}
          {session.status === 'archived' ? t('threads.unarchive') : t('threads.archive')}
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={() => void remove()} className="text-destructive focus:text-destructive"><Trash2 className="size-4" />{t('common.delete')}</DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
}
...
export function AppHeader() {
  const { viewMode, isMobile, draftThreadOpen, isSidebarOpen, setSidebarOpen, hasListPanel } = useLayout()
  const t = useT()
  const {
    files,
    selectedFileId,
  ...
  } else if (viewMode === "threads" && draftThreadOpen) {
    title = <h3 className="w-0 flex-1 truncate text-sm font-semibold">{t("threads.newThread")}</h3>
  ...
  return (
    ...
    >
      ...
      {hasListPanel && viewMode !== 'threads' && !isSidebarOpen && (
        <Tooltip>
      ...
      />
      {viewMode === 'threads' && !isMobile && !draftThreadOpen && <ThreadActions />}
    </header>
  )
}
