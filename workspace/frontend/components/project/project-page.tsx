...
export function ProjectPage({ projectId }: { projectId: string }) {
  ...
  return <WorkspaceProvider key={workspace.workspaceId} workspaceId={workspace.workspaceId} token={null} bearerToken={idToken} scopeFilter="project"><LayoutProvider embedded><ProjectInternalPage projectScoped projectId={workspace.workspaceId} projectName={workspace.name} onBack={() => router.push('/projects')} initialSessionId={search.get('session') || undefined} planStorageKey={`oa:project:${workspace.workspaceId}:plan:v1`} /></LayoutProvider></WorkspaceProvider>;
}
