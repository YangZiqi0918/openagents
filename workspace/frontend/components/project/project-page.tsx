...
const PLAN_TASK_VIEWS = new Set(['all', 'activity', 'comments', 'transition', 'history']);
...
export function ProjectPage({ projectId }: { projectId: string }) {
  ...
  const navigatePlanTask = useCallback((itemId: string, taskId: string, view = 'all') => {
    const next = new URLSearchParams(search.toString());
    next.set('tab', 'plan');
    next.set('item', itemId);
    next.set('task', taskId);
    next.set('view', PLAN_TASK_VIEWS.has(view) ? view : 'all');
    router.push(`/projects/${encodeURIComponent(projectId)}?${next.toString()}`);
  }, [projectId, router, search]);
  const closePlanTask = useCallback(() => {
    const next = new URLSearchParams(search.toString());
    next.delete('task');
    next.delete('view');
    const query = next.toString();
    router.push(`/projects/${encodeURIComponent(projectId)}${query ? `?${query}` : ''}`);
  }, [projectId, router, search]);
  const changePlanTaskView = useCallback((view: string) => {
    const next = new URLSearchParams(search.toString());
    next.set('view', PLAN_TASK_VIEWS.has(view) ? view : 'all');
  ...
  return <WorkspaceProvider key={workspace.workspaceId} workspaceId={workspace.workspaceId} token={null} bearerToken={idToken} scopeFilter="project"><LayoutProvider embedded><ProjectInternalPage projectScoped projectId={workspace.workspaceId} projectName={workspace.name} onBack={() => router.push('/projects')} initialSessionId={search.get('session') || undefined} initialTab={search.get('tab') === 'plan' || search.get('tab') === 'review' ? search.get('tab') as 'plan' | 'review' : undefined} initialPlanItemId={search.get('item') || undefined} initialPlanTaskId={search.get('tab') === 'plan' ? search.get('task') || undefined : undefined} initialPlanTaskView={PLAN_TASK_VIEWS.has(search.get('view') || '') ? search.get('view')! : 'all'} onOpenPlanTask={navigatePlanTask} onClosePlanTask={closePlanTask} onPlanTaskViewChange={changePlanTaskView} initialReviewTaskId={search.get('tab') === 'review' ? search.get('task') || undefined : undefined} planStorageKey={`oa:project:${workspace.workspaceId}:plan:v1`} /></LayoutProvider></WorkspaceProvider>;
}
