...
import { ArrowRightLeft, Check, Loader2, Plus, RefreshCw, Send, Trash2 } from 'lucide-react';
...
export function ServerPlanPage({ storageKey, focusItemId }: { storageKey: string; focusItemId?: string }) {
  ...
  const taskState = (status: string) => zh
    ? ({ backlog: '待接收', in_progress: '进行中', need_input: '待审核', done: '已完成' } as Record<string, string>)[status] ?? status
    : ({ backlog: 'To accept', in_progress: 'In progress', need_input: 'Awaiting review', done: 'Done' } as Record<string, string>)[status] ?? status;
    ...
    <div data-testid="project-plan-page" className="flex min-h-0 flex-1 flex-col bg-background px-5 pb-8 pt-5 sm:px-8 lg:px-10">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" onClick={previewOldPlan} disabled={busy}>{zh ? '导入本地计划' : 'Import local plan'}</Button>
