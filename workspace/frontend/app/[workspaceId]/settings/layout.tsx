...
function SettingsShell({ workspaceId, children }: { workspaceId: string; children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (IS_LOCAL_MODE) {
      ...
      setToken(null);
      requestLocalWorkspaceAccess(workspaceId, controller.signal)
        .then((access) => { if (!controller.signal.aborted) setToken(access.token); })
        .catch(() => { if (!controller.signal.aborted) setToken(''); });
  ...
  useEffect(() => {
    ...
    workspaceApi.configure(workspaceId, token, idToken || '');
    let cancelled = false;
